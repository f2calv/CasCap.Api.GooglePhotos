...
public class MyBackgroundService : BackgroundService
{
    ...
    {
        _logger.LogDebug("{ClassName} starting {MethodName}...", nameof(MyBackgroundService), nameof(ExecuteAsync));
        ...
        _logger.LogInformation("{ClassName} {Name} '{Title}' id is '{Id}'", nameof(MyBackgroundService), nameof(album), album.title, album.id);
        ...
        {
            _logger.LogInformation("{ClassName} album #{I} {FileName} {Width}x{Height}", nameof(MyBackgroundService), i, item.filename,
                item.mediaMetadata.width, item.mediaMetadata.height);
            i++;
        }

        _logger.LogDebug("{ClassName} exiting {MethodName}...", nameof(MyBackgroundService), nameof(ExecuteAsync));

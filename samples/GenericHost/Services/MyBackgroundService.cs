...
public class MyBackgroundService : BackgroundService
{
    ...
    protected async override Task ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        var albumMediaItems = await _googlePhotosSvc.GetMediaItemsByAlbumAsync(album.id, cancellationToken: cancellationToken).ToListAsync(cancellationToken) ?? throw new Exception("retrieve media items by album id failed!");
        var i = 1;
